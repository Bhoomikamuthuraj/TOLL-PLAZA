...
app.config['MAIL_PASSWORD'] = "ozvjhgvncoiswmyy"
...
def add_prebook():
    if request.method=="POST":
        ...
        cursor=mydb.cursor()
        cursor.execute("INSERT INTO final_project.pre_book_table(vehicle_no,toll_name,amount_paid,status) VALUES (%s,%s,%s,%s)",(number,select_toll,toll_amount,status))
        mydb.commit()
...
def detect():
    img="C:/Code/plate_images/plates/scanned_img1.jpg"
